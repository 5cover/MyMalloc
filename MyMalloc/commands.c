...
Command const *getCommandFromName(CommandGroup commands,
                                  char const *commandName);
Command const *getCommandFromParsedResult(CommandGroup commands,
...
CommandGroup createCommandGroup(Command const commands[], size_t count) {
  ...
  return (CommandGroup){
      .array = commands, .count = count, .maxNameLength = maxNameLength};
}
...
Command const *inputCommand(CommandGroup commands, long long *argument) {
  ...
  while (true) {
    ...
    Command const *command =
        getCommandFromParsedResult(commands, parsedCommand);
...
void showCommandMenu(CommandGroup commands) {
  foreach (Command const, command, commands.array, commands.count) {
    printf("%*s %s\n", -(int)min(INT_MAX, commands.maxNameLength),
