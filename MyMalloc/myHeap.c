...
// Alignement strategy.
// myAlloc will align allocations on the nearest multiple of the value
// evaluated. size represents the requested size in bytes.
#define ALIGN 1
...
void *myAlloc(size_t size) {
  ...
  if (gs_chunkCount == ARRAYLENGTH(gs_chunks)) {
    fprintf(stderr,
            "Allocation failed: maximum number of allocations (%zu) reached.\n",
  ...
  // Best: O(1)
  // Worst: O(n�)
  for (intptr_t start = alignedStart;
       start + (intptr_t)size <= HEAP_START_PTR + HEAP_SIZE; start += ALIGN) {
...
void myFree(void const *ptr) {
  ...
  if (!chunkFound) {
    // Freeing an invalid pointer is undefined behavior as per the C standard,
    // so we can do whatever we want here.

    fprintf(stderr, "Tried to free an invalid pointer: %p", ptr);
    // We could ignore the error, but it's probably unsafe to continue, so
    // fail-fast.
    abort();
  }
}

// Checks if at least one chunk occupies at least one byte of the specified
// memory area.
bool isAreaAllocated(intptr_t start, size_t size) {
  foreach (Chunk const, chunk, gs_chunks, gs_chunkCount) {
    if (rangesOverlap(chunk->start, chunk->start + chunk->size - 1, start,
                      start + size - 1)) {
      return true;
    }
  }
...
void heapDumpChunksConsole(void) {
  // Print chunk list
  printf("Chunks (%zu/%zu):\n\n| %-2s | %-16s | %-16s |\n", gs_chunkCount,
         CHUNKS_LENGTH, "#", "Start offset", "Size");
  size_t totalSize = 0;
  for (size_t i = 0; i < gs_chunkCount; ++i) {
    Chunk const chunk = gs_chunks[i];
    printf("| %-2zu | %-16ld | %-16zu |\n", i, chunk.start - HEAP_START_PTR,
           chunk.size);
...
void heapDumpChunksBitmap(char const *filename) {
  ...
  generateBitmapImage((uint8_t const *)image, DUMP_BMP_HEIGHT, HEAP_SIZE,
                      filename);
}
